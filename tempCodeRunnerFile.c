...
// linked list switch 2 nodes
void xchg(struct Node *a, struct Node *b, struct node **h, struct node **t) {
  struct Node *hc = h;
  struct Node *tc = t;

  struct Node *current = a; 
  while (current -> next != b -> data) {
    current = current -> next;
  }
  struct Node *temp = a; 
  a = current -> next; // a now points to value of b

  struct Node *currentb = b; 
  while (currentb -> previous != temp -> data) { // temp is now a, we already changed a
